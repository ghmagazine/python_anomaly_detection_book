...
実はベータ二項分布は、成功確率パラメータ$\mu$の事前分布をベータ分布として、二項分布をベイズ推定した場合の予測分布でもあります。

ベータ二項分布モデルのベータ分布は、成功確率パラメータ$\mu$の過分散を明示的に表現することを目的として、モデルに導入されていました。一方でベイズ推定の事前分布は、成功確率パラメータ$\mu$の事前知識のあいまいさを表現することを目的としており、過分散を明示的に表現しているわけではありません。すなわちこの2種類のモデルは、予測分布の確率分布の種類は同じだが、ベータ分布の導入目的が異なると言えるでしょう。
...
　この予測分布は式(8.103)と同じ形のベータ分布と二項分布の合成分布、すなわちベータ二項分布$BetaBin(x \mid n,\alpha+m,\beta+m)$。データが増えて$m,n$が大きくなるほど、ベータ分布の分散は小さくなるため、データ不足による推定のあいまいさ（分散）がデータが増えるごとに縮小していくことが式からも確認できます。
　式からも明らかなように二項分布のベイズ推定では、データが増えると予測分布が通常の二項分布に収束していくため、データの多寡とは無関係に発生している過分散を表現することはできないので、ご注意ください。

## 参考：負の二項分布モデルとポアソン分布のベイズ推定

### ポアソン分布

　分母が一定のポアソン分布に関して、期待発生回数パラメータ$\mu$の共役事前分布$p(\mu)$として、以下のガンマ分布を採用した場合のベイズ推定を考えます。

```math
p(\mu)=Ga(\mu \mid \theta_0,k_0)
\tag{5.69}
```
　
　このとき、$\mu$の事後分布は以下のガンマ分布として求まります。

```math
p(\mu \mid \boldsymbol{X})=Ga\Bigl(\mu \mathrel{} \Bigl| \mathrel{} \frac{\theta_0}{\theta_0 + 1},k_0+m\Bigr)
\tag{5.70}
```

　ただし、$m$は学習データ中の発生数$x$の合計を示し、以下のように表されます。
...
　$\theta_N=\frac{\theta_0}{\theta_0 + 1}$、$k_N=k_0+m$とおくと、予測分布は以下の負の二項分布として求まります。

```math
\begin{aligned}
p(x \mid \boldsymbol{X})
&=
NB \Bigl(x \mathrel{} \Bigl| \mathrel{} k_N, \frac{1}{\theta_N + 1}\Bigr) \\
&=
\binom{x+k_N-1}{k_N-1} \Bigl(\frac{1}{\theta_N + 1}\Bigr)^{k_N} \Bigl(1-\frac{1}{\theta_N + 1}\Bigr)^x
\end{aligned}
\tag{5.72}
```

　なお負の二項分布には複数の定義が存在しますが、本書ではScipyでの定義[^scipy_nbinom]に従います。

[^scipy_nbinom]:https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.nbinom.html

### ポアソン分布（分母が一定でない場合）

　「4.5.3 ポアソン分布の最尤推定による異常検知」における式 (4.125)のように、分母（となる時間や面積）$t$が一定でない場合のポアソン分布を考えます。詳細な式変形は「補足 」を参照ください。

　$i$番目のデータにおける分母（となる時間や面積）を$t^{(i)}$、発生数を$x^{(i)}$とすると、単位あたり平均発生数パラメータ$\mu$の事後分布は以下のガンマ分布として求まります。

```math
p(\mu \mid \boldsymbol{t},\boldsymbol{X})=Ga\Bigl(\mu \mathrel{} \Bigl| \mathrel{} \frac{\theta_0}{\theta_0 l + 1},k_0+m\Bigr)
\tag{5.73}
```

　ただし、$m$は式 (5.71)で表され、$l$は学習データ中の分母（となる時間や面積）の合計を示し、以下の式で表されます。
...
　$\theta_N=\frac{\theta_0}{\theta_0 l + 1}$、$k_N=k_0+m$とおくと、予測分布は以下の負の二項分布として求まります。

```math
\begin{aligned}
p(x \mid t,\boldsymbol{X})
&=
NB \Bigl(x \mathrel{} \Bigl| \mathrel{} k_N, \frac{1}{\theta_N t + 1}\Bigr) \\
&=
\binom{x+k_N-1}{k_N-1} \Bigl(\frac{1}{\theta_N t + 1}\Bigr)^{k_N} \Bigl(1-\frac{1}{\theta_N t + 1}\Bigr)^x
\end{aligned}
\tag{5.75}
```
